...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-8 max-w-lg">
      <header>
        <h1 className="text-3xl font-display font-semibold text-oxblood-700">Settings</h1>
        <p className="text-charcoal-light mt-1">Small preferences, so the app feels right.</p>
      </header>

      <section aria-labelledby="theme-heading">
        <h2 id="theme-heading" className="text-lg font-display font-semibold text-oxblood-700 mb-3">Theme</h2>
          ...
          {MODES.map(({ value, label, icon: Icon }) => (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-1.5 rounded-lg border py-3 text-sm font-medium transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-crimson-500 ${
                mode === value ? 'border-crimson-400 bg-blush-100 text-oxblood-700' : 'border-oxblood-100 text-charcoal-light hover:bg-blush-50'
              }`}
            >
      ...
      <section aria-labelledby="secret-heading">
        <h2 id="secret-heading" className="text-lg font-display font-semibold text-oxblood-700 mb-3">Secret mode</h2>
        <Card>
          {secretUnlocked ? (
      ...
      <section aria-labelledby="status-heading">
        <h2 id="status-heading" className="text-lg font-display font-semibold text-oxblood-700 mb-3">Connection</h2>
        <Card>
